        ...
        for self.root, dirnames, filenames in os.walk(self.directory):
            for filename in filenames:
                try:
                    ...
                    if len(contents) > 0:
                        doc.add(Field("contents", contents, TextField.TYPE_STORED))
                    else:
...
class Queryer():
    ...
    def run(self, writer=None, analyzer=None):
        ...
        searcher = IndexSearcher(DirectoryReader.open(SimpleFSDirectory.open(File(self.store_dir))))
        while True:
            print
            ...
            query = QueryParser(Version.LUCENE_43, "contents", analyzer).parse(command)
            ...
            highlighter = Highlighter(SimpleHTMLFormatter('<b><font color="red">', '</font></b>'), QueryScorer(query))

            # Using NullFragmenter since we still want to see the whole document
