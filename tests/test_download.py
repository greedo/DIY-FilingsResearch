...
ping_edgar = pytest.mark.skipif(os.system("ping -c 1 " + Edgar().org_root.replace("http://", "")) != 0,
...
def test_sedar_ingest_xbrl():
    ...
    initial_request = session.post(Sedar().org_root+"/FindCompanyDocuments.do", params=initial_params)
